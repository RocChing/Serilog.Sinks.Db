...
namespace Serilog.Sinks.Db
{
    public partial class ColumnOptions
    {
        ...
        internal void FinalizeConfigurationForSinkConstructor()
        {
            ...
            {
                Console.WriteLine("Deprecated: The \"AdditionalDataColumns\" collection will be removed in a future release. Please use the \"AdditionalColumns\" collection.");
            ...
            if (ClusteredColumnstoreIndex)
            {
                ...
                {
                    PrimaryKey = null;
                    Console.WriteLine("Warning: Removing primary key, incompatible with clustered columnstore indexing.");
                }
            ...
            {
                Console.WriteLine($"Warning: Primary key must be NON-NULL, changing AllowNull property for {PrimaryKey.ColumnName} column.");
        ...
        private void ColumnstoreCompatibilityCheck(SqlColumn column)
        {
            ...
            if (column.DataLength == -1 && SqlDataTypes.DataLengthRequired.Contains(column.DataType))
                Console.WriteLine($"Warning: SQL2017 or newer required to use columnstore index with MAX length column \"{column.ColumnName}\".");
        }
    }
}
